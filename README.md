# Identifying genetic variants in a pangenome graph
...
We recommend using `pantree` with graphs generated using Minigraph-Cactus. Graphs generated using `vg` permit the linear reference walk to contain cycles, and this makes linear-reference coordinates ambiguous; be cautious with downstream pipelines that rely on the `POS` column, and prefer to identify variants using node IDs instead of reference coordinates. Additionally, the `--no-missingness` flag is recommended: Pantree annotates variants as 'missing' based on their positions along the reference genome, comparing these positions with the coordinates of contiguous walks, but this logic is fragile when the coordinates themselves are ambiguous.
...
```bash
uv run pantree consolidate <vcf_file> <sample_name> <haplotype_number> <output_path>
```
...
You can simplify a pangenome graph by removing small variants, tips, and contracting paths, then output in GFA format:

```bash
uv run pantree simplify input.gfa simplified.gfa --min-allele-length 1000
```

The simplified GFA includes node metadata that maps each new segment ID back to constituent segment IDs in the
input graph.

For concepts, complete command examples, and file-format contracts, see the
